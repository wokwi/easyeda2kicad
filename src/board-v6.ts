...
export function reportError(
  ...
  if (conversionState.convertingFpFile) {
    ...
    return reportFpError(conversionState.msgRepCnt, msg, lines, conversionState);
  } else {
...
export function convertPadToVia(
  args: string[],
  conversionState: IConversionState,
  parentCoords?: IParentTransform
): ISpectraList {
...
function getDrill(
  ...
  if (holeRadius && holeLength) {
    if (holeLength > holeRadius && height > width) {
      return ['drill', 'oval', holeRadius, holeLength];
    } else {
      return ['drill', 'oval', holeLength, holeRadius];
    }
  }
  if (holeRadius) {
...
export function convertPad(
  ...
  // EasyEda sometimes has small hole (0.001) defined in smd pad;
  // prevent conversion to Kicad thru_hole.
  // Assume drills are 0.2mm or larger
  let radius = holeRadius;
  if (kiUnits(holeRadius) > 0 && kiUnits(holeRadius) < 0.2) {
    radius = '0';
  }
  return [
    '_LF1_',
    [
      ...
      kiUnits(radius) > 0 ? 'thru_hole' : 'smd',
  ...
  */
  ...
  for (shape of board.shape) {
    ...
    if (type === 'VIA') {
      ...
      padvias.push(...convertPadToVia(args, conversionState));
    } else if (type === 'RECT') {

...
function convertPadToVia(args: string[], nets: string[], parentCoords?: IParentTransform) {
  ...
  if (shape === 'POLYGON') {
    console.warn('Conversion from Polygon via not supported yet');
    return;
  }
  ...
  if (shape !== 'ELLIPSE') {
    return [
      'module',
      'AutoGenerated:MountingPad_' + size.toFixed(2) + 'mm',
      ...
      [
        ...
        shapes[shape],
        ['at', 0, 0],
        ['size', kiUnits(holeRadius), kiUnits(height)],
        ['drill', slotted, drillHoleLength, kiUnits(drill) * 2],
        ['layers', '*.Cu', '*.Mask']
      ]
    ];
  }
